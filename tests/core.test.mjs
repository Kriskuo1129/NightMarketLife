...
assert.equal(Object.hasOwn(gameState.player, "score"), false);
assert.equal(gameState.progress.actionCount, 2);
assert.equal(Object.hasOwn(applyActivityResult({ scoreDelta: 90, progressCost: 0 }), "scoreDelta"), false);
...
assert.deepEqual(TEST_GAME_RESULTS.game_01, { staminaDelta: -10, moneyDelta: 50, completed: true, progressCost: 1, sourceId: "game_01" });
assert.deepEqual(TEST_GAME_RESULTS.game_02, { staminaDelta: -10, moneyDelta: 100, completed: true, progressCost: 1, sourceId: "game_02" });
assert.deepEqual(TEST_GAME_RESULTS.game_03, { staminaDelta: -10, moneyDelta: 20, completed: true, progressCost: 1, sourceId: "game_03" });
assert.equal(playTestGame("game_01").sourceId, "game_01");
assert.deepEqual([gameState.player.stamina, gameState.player.money], [90, 1050]);
assert.equal(playTestGame("game_02").sourceId, "game_02");
assert.deepEqual([gameState.player.stamina, gameState.player.money], [80, 1150]);
assert.equal(playTestGame("game_03").sourceId, "game_03");
assert.deepEqual([gameState.player.stamina, gameState.player.money], [70, 1170]);
...
assert.deepEqual([gameState.player.stamina, gameState.player.money], [100, 950]);
...
assert.equal(playTestGame("game_01", () => 0).moneyDelta, 60);
assert.equal(gameState.player.money, 60);
assert.deepEqual(applyRewardModifier({ moneyDelta: -50 }, gameState.environment), { moneyDelta: -50 });
assert.deepEqual(applyRewardModifier({ moneyDelta: 11 }, gameState.environment), { moneyDelta: 13 });
...
// Step 8 Money-First model and per-stall flow.
assert.equal(Object.hasOwn(createPlayer(), "score"), false);
createNewGame({ buildId: "worker" });
assert.equal(gameState.session.startingMoney, 1000);
assert.deepEqual(gameState.statistics.stallMoneyFlow, {});
gameState.session.openingPending = false;
gameState.progress.nextEventAt = 999;
playTestGame("game_01"); clearActivityResultPresentation();
playTestGame("game_02"); clearActivityResultPresentation();
assert.equal(gameState.statistics.stallMoneyFlow.game_01, 50);
assert.equal(gameState.statistics.stallMoneyFlow.game_02, 100);
applyActivityResult({ moneyDelta: 100, sourceId: "game_01" }, { trackStallMoney: true }); clearActivityResultPresentation();
assert.equal(gameState.statistics.stallMoneyFlow.game_01, 150);
applyActivityResult({ moneyDelta: -100, sourceId: "food_01" }, { trackStallMoney: true }); clearActivityResultPresentation();
applyActivityResult({ moneyDelta: -200, sourceId: "food_01" }, { trackStallMoney: true }); clearActivityResultPresentation();
assert.equal(gameState.statistics.stallMoneyFlow.food_01, -300);
applyActivityResult({ moneyDelta: 77, sourceId: "management" });
assert.equal(gameState.statistics.stallMoneyFlow.management, undefined);
applyActivityResult({ moneyDelta: 77, sourceId: "game_03" });
assert.equal(gameState.statistics.stallMoneyFlow.game_03, undefined);
assert.equal(gameState.session.startingMoney, 1000);
const { formatMoneyDelta } = await import("../js/ui.js");
assert.equal(formatMoneyDelta(300), "+$300");
assert.equal(formatMoneyDelta(-300), "-$300");
assert.equal(formatMoneyDelta(0), "$0");
for (const stall of gameState.stalls.filter(s => !s.isSpecial)) stall.isClosed = true;
assert.equal(gameState.session.scene, "NIGHT_MARKET");
gameState.player.stamina = gameState.player.money = 0;
assert.equal(gameState.session.scene, "NIGHT_MARKET");
assert.equal(gameState.achievements.length, 17);
assert.equal(gameState.achievements.some(a => ["HOT_HAND", "NIGHT_MARKET_LEGEND"].includes(a.id)), false);
