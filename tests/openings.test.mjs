...
for (const [i,o] of OPENING_CONFIG.entries()) {
  ...
  for (const build of CONFIG.characterBuilds) {
    ...
    assert.deepEqual([gameState.player.stamina,gameState.player.maxStamina,gameState.player.money],[build.stamina,build.stamina,build.money]);
    assert.equal(gameState.session.startingMoney, build.money);
...
assert.equal(getStallActivity(gameState.stalls.find(s=>s.id==='game_01'),gameState.environment).moneyDelta,60);
...
const initialResources=[gameState.player.stamina,gameState.player.money];
assert.equal(selectHomeBuild('college'),true);
assert.equal(loadCharacterSettings().buildId,'college');
assert.deepEqual([gameState.player.stamina,gameState.player.money],initialResources);
...
gameState.player.stamina=1; gameState.player.money=2;
startNightMarketFromHome('大學同學',()=>.75);
assert.equal(gameState.session.openingConditionId,'ANNIVERSARY');
assert.deepEqual([gameState.player.stamina,gameState.player.maxStamina,gameState.player.money],[110,110,800]);
const setting=loadCharacterSettings();
for(const key of ['stamina','money','score','startingMoney','openingConditionId','openingPending']) assert.equal(Object.hasOwn(setting,key),false);
